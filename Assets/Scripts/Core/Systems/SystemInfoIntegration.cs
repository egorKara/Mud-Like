...
{
    /// <summary>
    /// Интеграция с существующими системными утилитами Cursor
    /// Использует htop, /proc/cpuinfo, /proc/loadavg и другие системные источники
    /// </summary>
    public static class SystemInfoIntegration
    {
        /// <summary>
        /// Получить информацию о системе через доступные утилиты
        /// </summary>
        public static SystemInfo GetSystemInfo()
        {
            ...
            try
            {
                ...
                // Получаем информацию о температуре (если доступно)
                ...
                Debug.Log($"[SystemInfo] CPU: {info.CPUModel}, Cores: {info.CPUCores}, Load: {info.CPULoad:F1}%");
                Debug.Log($"[SystemInfo] RAM: {info.UsedRAM / 1024 / 1024}MB/{info.TotalRAM / 1024 / 1024}MB ({info.RAMUsage:F1}%)");
                Debug.Log($"[SystemInfo] Temperature: {info.CPUTemperature:F1}°C, Uptime: {info.Uptime:F1}h");
            }
            catch (System.Exception e)
            {
                Debug.LogError($"[SystemInfo] Ошибка получения информации о системе: {e.Message}");
            }
        ...
        private static string GetCPUModel()
        {
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения модели CPU: {e.Message}");
            }
        ...
        private static int GetCPUCores()
        {
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения количества ядер: {e.Message}");
            }
        ...
        private static float GetCPULoad()
        {
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения нагрузки CPU через htop: {e.Message}");
        ...
        /// </summary>
        private static float GetCPULoadFromProc()
        {
            try
            {
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения нагрузки из /proc/loadavg: {e.Message}");
            }
        ...
        private static long GetTotalRAM()
        {
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения общего объема RAM: {e.Message}");
            }
        ...
        private static long GetUsedRAM()
        {
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения используемой RAM: {e.Message}");
            }
        ...
        private static float GetCPUTemperature()
        {
                    ...
                    foreach (string zone in thermalZones)
                    {
                        ...
                        if (File.Exists(tempFile))
                        {
                            ...
                            if (int.TryParse(tempStr, out int tempMilliCelsius))
                            {
                                ...
                                {
                                    Debug.Log($"[SystemInfo] Найдена температура: {tempCelsius:F1}°C");
                                    return tempCelsius;
                                }
                            }
                        }
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения температуры: {e.Message}");
                return EstimateTemperatureFromLoad();
            }
        }
        ...
        private static float GetSystemUptime()
        {
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения времени работы: {e.Message}");
            }
        ...
        /// </summary>
        private static float GetLoadAverage()
        {
            try
            {
                if (File.Exists("/proc/loadavg"))
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка получения средней нагрузки: {e.Message}");
            }
        ...
        /// Парсинг вывода htop
        /// </summary>
        private static float ParseHtopOutput(string output)
        {
            try
            {
            ...
            {
                Debug.LogWarning($"[SystemInfo] Ошибка парсинга htop: {e.Message}");
            }
        ...
        private static float EstimateTemperatureFromLoad()
        {
            try
            {
                ...
                Debug.Log($"[SystemInfo] Оценочная температура: {estimatedTemp:F1}°C (нагрузка: {cpuLoad:F1}%)");
        ...
        /// </summary>
        public static string GetAvailableUtilities()
        {
